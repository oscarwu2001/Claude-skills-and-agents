---
name: archify
description: "Draw a system as a validated, self-contained interactive HTML diagram — architecture, workflow, sequence, data-flow, or lifecycle/state — with dark/light themes, PNG/SVG/WebM export, and optional trace motion. Use when the user wants to visualize or diagram architecture, infrastructure, or cloud/network/security topology; a technical workflow or runbook; an API call sequence or request lifecycle; a data pipeline, ETL, or lineage; a state machine; to compare architecture before and after a change; or to convert and beautify Mermaid. Accepts plain-language requirements or pasted Mermaid, and inspects repository evidence when the diagram must reflect real code. To answer questions about a codebase rather than draw it, use graphify. This is system topology, not quantitative charts — for plotting data, metrics, dashboards, or any chart of numbers, use dataviz instead."
license: MIT
metadata:
  version: "2.16"
  author: tt-a1i
...
<!-- LOCAL INTEGRATION — not upstream. Recorded in UPSTREAM.md; re-apply after updating. -->
## Where this skill lives

Every relative path in this skill — `bin/`, `schemas/`, `examples/`, `references/`, `recipes/` — is relative to **the skill directory, not the user's project**. Your working directory is the project, so pin the skill directory once per session and prefix every archify path with it:

```bash
ARCHIFY="$HOME/.claude/skills/archify"        # bash
```
```powershell
$ARCHIFY = "$HOME\.claude\skills\archify"     # PowerShell
```

Read `$ARCHIFY/schemas/…` and `$ARCHIFY/examples/…`; invoke the CLI as `node "$ARCHIFY/bin/archify.mjs" <command>`. **Only the skill's own files take the prefix** — input specs and output HTML stay relative to the project, and generated `.json` / `.html` belong in the project, never in the skill directory. Confirm the install any time with `node "$ARCHIFY/bin/archify.mjs" doctor`.

## Working with the other installed skills

- **`graphify`** answers questions about a codebase; archify draws one. When `graphify-out/graph.json` exists, get the topology from it — `graphify query "what are the top-level components and how do they call each other"` — and author the spec from that answer rather than guessing. A diagram of a real repo should be grounded in evidence, not inferred from filenames.
- **`/code-review`** and **`/improve-codebase-architecture`** want the Before/Delta/After artifact from `compare` whenever a change moves architecture.
- **`/grill-with-docs`**, **`/to-spec`**, and **`/wayfinder`** are where a diagram earns its keep as an argument — draw the proposed design so the decision is visible, and link the HTML from the spec or ADR.
