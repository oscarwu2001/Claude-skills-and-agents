...
- **Provenance discipline:** most skills here are editable copies of
  upstreams, tracked in `~/.claude/skills/UPSTREAM.md`. Before staging an
  edit to ANY installed skill, read that file. For the weekly review's
  Step 2 classification these skills are category (a) — edits survive —
  but ONLY when the edit follows the house convention: body changes
  wrapped in `<!-- LOCAL INTEGRATION -->` markers, and every edit
  (including frontmatter `description:` rewrites, which cannot carry
  markers) recorded in UPSTREAM.md's Local edits tables. An unmarked,
  unrecorded edit behaves like category (b): the next manual upstream
  sync silently destroys it. Update UPSTREAM.md in the same turn as the
  staged edit — the two are one act.
- **Authoring craft:** when drafting or substantially editing a skill,
  also load the installed `writing-great-skills` skill.
...
- **Boundary with Claude Code auto-memory:** the memory directory stores
  ...
  rule, step or edge case in *an installed skill* → this observation log.
  When it is genuinely both, write both — they index different things.
